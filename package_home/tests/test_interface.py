...
test_topics_path = os.path.join(current_dir, 'test_topics.csv')
print(f"Looking for file at: {test_topics_path}")
print(f"File exists: {os.path.exists(test_topics_path)}")
# Konu analizi için kaydedilen konu kelimeleri yüklemesi 
def test_load_topics():
    topics = {}
    with open(test_topics_path, 'r', encoding='utf-8') as file:
...
class AudioHistogramApp:
    def __init__(self, root):
        ...
        self.toggle_button = ttk.Button(
            root, text="Start", command=self.test_toggle_recording
        )
    ...
    def test_toggle_recording(self):
        if not self.is_running:
            ...
            self.test_record_audio_loop()
            threading.Thread(target=self.test_speech_to_text_loop, daemon=True).start()
        else:
            self.is_running = False
    ...
    def test_record_audio_loop(self):
        if self.is_running:
            self.test_update_visualizations()
            self.update_id = self.root.after(100, self.test_record_audio_loop)
    ...
    def test_speech_to_text_loop(self):
        while self.is_running and not self.stop_recognition:
            try:
                ...
                if self.is_running:
                    self.root.after(0, self.test_update_text, text)
    ...
    def test_calculate_metrics(self, true_labels, predicted_labels):
